...
- feat(talk): agents decide for themselves, and start chats of their own. `list_agents` says what each chat is doing — working for how long, on what, running which tool, how many prompts queued behind it, answering another agent, waiting on an answer or on the user — and `message_agent` takes `delivery`: `interrupt` (the default) or `queue`; a message that couldn't interrupt tells its sender why. `start_chat` (`{"do": "new"}` over HTTP) opens a new chat in any open project, or a folder by its path, optionally on a model of the agent's choosing, and sends it a first message whose answer comes back like any other — a fresh agent to ask about a codebase. `talk-replies-test` runs all three against real Claude and Codex.
- feat(talk): a message from another agent interrupts a chat at work instead of queuing behind it. The running turn stops, the message goes in at once (ahead of the user's queue, which follows), its bubble says "cut in", and the model deals with it — the turn is the message's alone, so an answer is exactly its last words — and then a prompt from ruri sends it back to what it was doing, before the user's queue. It still waits in line when the turn is answering another agent that wants the answer, when the chat is waiting on an answer of its own, or when it is waiting on the user at a card. `talk-replies-test` has a case for it: a chat ninety seconds into a command is messaged, answers in seconds, and takes its command up again (`TALK_REPLIES_CASES=8` runs just that).
- fix(talk): an agent's message always gets its answer back, unless it asked for none — and waiting for it works on every harness. A wait used to be one call held for up to twenty minutes, which a harness's shell cuts off long before: Codex hands a command back after thirty seconds whatever it is asked, OpenCode kills one at two minutes, Gemini one silent for five. The answer then landed in a call nobody was reading, and once the sender's turn ended it was dropped outright — so was every answer to a sender whose wait the user stopped. A wait is now held in slices each harness can hold (ten minutes of Claude's tool call; twenty seconds on Codex, ninety on OpenCode, four minutes on Gemini), each ending "not yet" with the message's letter id, and `wait_for_answer` — `{"do": "wait", "letter"}` over HTTP — waits again. An answer that finds no call holding for it goes to the sender's chat as a message, where a wait coming back for it still takes it out of the line; the letters in flight are kept on disk, so a relaunch sends on one still waiting in the other chat's line, delivers an answer that had landed, and reports a turn that never finished with what it had said. `POST /talk` waits by default, as the tool does; "none" is the fire-and-forget, and the other agent is told nobody is waiting. `talk-replies-test` runs it all against Claude and Codex (and OpenCode) with the other chat kept busy.

- fix(sessions): a chat can't lose its session, however often its model moves between harnesses. A switch from Claude to Codex (or OpenCode) started the new harness on a conversation it had never heard — asked for a codeword planted on Claude, Codex answered only its own — and a switch back started Claude over too ("None. This is the start of our conversation."), since the chat kept one session id and the harness it left overwrote it. A chat now keeps a session on every harness it has run on: the one it comes back to is resumed, told only the exchanges that ran elsewhere while it was away, and a harness it has never run on starts from a brief of the whole conversation. What each session holds moves on only when a turn on it ends, so nothing is spent by a start that fails: quitting or crashing the app after a `/compact` before the fresh session's first reply used to take the brief with it for good, and now the next prompt is told again. A rewind's fork point waits for its fork to be made (a build on another harness, or a start cut off by a quit, used to spend it and resume the old session at its tip, rewound exchanges and all); a rewind keeps each harness's session only if nothing it took out went to it. A switch never retires a turn that is running. A Codex thread or OpenCode session that is gone ("no rollout found", or an ACP agent answering with a new session) heals the way a lost Claude session does — a fresh session, briefed, the prompt sent again — instead of failing every prompt after. Session ids and fork points are written the moment they change; the server writes everything down before it goes on SIGTERM or an uncaught exception, as it did on SIGINT; and Home's kickoff prompt goes through the same door as a typed one — the chat's own model, its queue, its catch-up. `bun run session-stress-test` does all of this to one chat against the real CLIs, asking for its codewords after every step.
- fix(gauges): the dragons always show their numbers — over an empty chat too, and in a narrow window they shrink instead of vanishing. A chat switched onto plain Opus 5.5 no longer draws its context dragon full: each model's window is read back from what earlier turns reported, so a relaunch still knows Opus has 1M, and a chat already past 200k is never measured against 200k. Claude's models keep their versioned names ("Opus 5.5", not "Opus") when the Home session reports its list before the startup catalog lands.
- fix(models): "Opus 1M" is Opus 5.5 again, measured against the window it has. The Claude CLI stopped listing its `[1m]` models once the plain ones had the million-token window themselves (`opus` reports 1,000,000, as `opus[1m]` does), so a default left on `opus[1m]` sat on a row named from its id. Stars and roles on a `[1m]` id the CLI no longer lists now move to the plain model it does. And a Claude chat's context gauge takes the window the CLI reports with each turn, rather than guessing 200,000 for any id without `[1m]` — which read a chat on plain Opus 5.5 as five times fuller than it was. `model-window-test` checks both against the real CLI.
- fix(settings): a model's role tag can't be lost by unstarring it. Unstarring the small-tasks model or the default released the role, and the tag vanished with nothing left to drag it back from; a model wearing a tag now keeps its star until the tag goes to another model. A starred model or a role's holder also keeps its row in the catalog when its harness stops listing it — the Claude CLI no longer lists `[1m]` models, which had taken an `opus[1m]` default's tag off the page.
- feat(models): every model wears its maker's doodle — beside its name in the composer's model picker and on the picker itself, in the agents page's picker and in Settings' model list: Claude's spark, ChatGPT's tile, Gemini's sparkle and the rest, the lab's mark where the product is only a name, the harness's own when no lab is known. The rule under the peek band is solid; it read as dotted where the screentone showed through it.
- feat(chat): over an empty chat, the marks of whoever made its model — doodled in felt-tip: Anthropic × Claude, OpenAI × ChatGPT, Google × Gemini, xAI × Grok, Moonshot × Kimi, Alibaba × Qwen, Z.ai × GLM, Mistral × Le Chat, Meta × Llama, DeepSeek's whale and MiniMax beside their names lettered by hand, and for a model no lab claims, its harness's mark (OpenCode, Goose) × its name. The lab is read off the model — an OpenCode provider that names one, the model's own name, or a harness that only runs its maker's models — so OpenCode's own gateway serving Muse Spark shows Meta's. The lines follow the theme's ink, the brand colours sit a hair off them, and picking another model before the first prompt changes them.
- feat(chat)!: the hero face is gone — the cropped manga panels over an empty chat, Settings → Hero face and its editor, and the pictures themselves. A fresh chat keeps its title and Home its greeting; the greetings move to **Settings → Greeting** (one line always said, several taking turns a launch at a time, none for no title), kept as their own preference and read from the old one until the first change, so a list you set carries over.
- feat(band): the peek band changes with the theme. A fresh install's band is one mountain path — by day on light, under the stars on dark, at sunset on ember — and any picture in Settings → Peek band can be kept to light, dark or ember, or any two, so a band of your own can turn with the theme too. The hand-cut heads it used to come with are gone, from the app and from a band that still named them; **Restore the default** puts the mountain path back. The app icon is the thinking indicator's dragon on a paper tile (`build/icon.svg`, `make icon`), and the README banner is the same dragon in a panel. The art tuner (`make tuner`) is gone with the art it placed.
- feat(architecture): the stack is an index, and each layer has a sheet of its own. Every session is shown the stack before it starts — a line a layer, with its handle — and reads the sheet of the layer it's about to work in (`.ruri/layers/<handle>.md`, or `ruri layer <handle>`): where to change what inside it, how it works, its key files, its traps, what it talks to. The repo read cuts any layer owning more than 30 files into its parts and places every file in a layer; turns fold only into the sheets of the layers whose files they changed, so the architecture stays short however big the project gets. On the Architecture page each stack bar opens its layer's sheet, every line correctable. A project with an older sheet is read afresh the first time one of its turns is folded.
- fix(sessions): "opencode: ACP connection closed" no longer fails a prompt. OpenCode keeps everything in one SQLite database, and when two start in the same moment — a chat's and the small model titling its prompt, both on OpenCode — one can find it locked and exit on the way up; a warm process could also die while the chat sat idle. Either way the agent never heard the prompt, and the next one worked. A turn whose harness process dies before saying anything now opens it again, on the session it had, and sends the same prompt — twice at most, never after a stop, and never for a sign-in or a missing harness. `acp-start-test` runs both against the real OpenCode.
- fix(sessions): "No conversation found with session ID" no longer ends a turn. A resume that fails as it starts — the session's file gone, or a rewind's fork point not in it — hands its prompt to a fresh session briefed on the conversation, the way a `/compact` would, and the chat carries on with a line saying so. The check before a prompt now also lets go of a process that died holding the old id (the next build used to resume that id again, straight past the check), and looks for Claude's transcripts under a project's real path, so a folder reached through a symlink (`~/Workspace`) is checked too. `lost-session-test` runs both failures against the real CLI.
...
- fix(sessions): a chat no longer dies with "No message found with message.uuid" after a rewind and a `/compact`. A rewind's fork point was kept per chat with nothing saying which Claude session it was a point in; `/compact` moved the chat to a new session and left the point behind, and the next time its process was rebuilt — here, an agent's answer arriving — the new session was resumed at the old one's message. The point now names its session, is handed only to a build resuming that session, is taken by whatever builds next either way, and goes when the chat leaves the session; a start that fails before any turn no longer leaves the chat resuming the fork it never made; and before a prompt resumes a Claude session, the session and its fork point are checked against Claude's own transcripts — one that isn't there is let go of for a brief of the conversation, rather than failing every prompt after. Points saved by earlier versions, which name no session, are dropped.
- feat(memory): the catch-up leads with git's account and every line says where it came from. `.ruri/catchup.md` opens with the branch, what's uncommitted, what's unmerged and the last commits, read from git as it is written and again after every turn; an open item naming a branch says what git makes of it. Each memory line carries the day it was learned (in your time, not UTC's), the exchange it came from (`7a3637b4#16`) and who wrote it. Agents write down what they learn first-hand with `ruri note decision "…" --why "…"` (or `failed`, `worked`, `trap`, `open`), list and retire lines with `ruri memory` / `ruri forget`, search every exchange in the project with `ruri recall <words>`, print one whole with `ruri recall show <ref>`, and read git and their own changes with `ruri state`. The small model keeps lines by id, exactly as stored, and rewords only its own — never an agent's or yours.
- feat(compaction): a brief that leads with the state and weighs the recent. It opens with git's account, the files the conversation changed and what the last reply offered to do next; the last three exchanges come at length, your words as you wrote them; and as it goes out it adds the earlier exchanges and memory lines that share your next prompt's words — no model call, so the send stays instant.
- feat(architecture): where to change what, and a page you correct. The architecture sheet leads with the things a session is likely to be asked to change and the files each lives in — drawn from every source path in the repo, grown from the files turns change, only ever real files — and says how many commits ago the repo was read. On the page, hover any line to rewrite or strike it, pin a memory line so nothing rewrites it, or add your own with a card's +; your lines are pinned.
- feat(memory): a project's catch-up is its working memory now, not a list of features. `.ruri/catchup.md` holds where the work stands, the decisions made and why, what worked, what was tried and failed and why, the gotchas and rules, and what's still open — dated, merged and pruned by the small model from every chat's finished turns (read long enough to catch what an agent tried mid-turn), and written from the chats' histories whole the first time. The project's shape moved to `.ruri/architecture.md`: the purpose, the stack as layers from the top down, the paths through the system as flows, features, where things are, how to run it, conventions — written from the repo, folded forward from work. Every session is pointed at both, in words that never change. The new Architecture page in the chat header draws them — the stack as bars, flows as boxes and arrows, the memory in cards — with Read the repo and Read the chats. The per-chat compaction digest keeps decisions with their reasons and what failed, too
- feat(components): a component library per project. The Components page is a gallery under the project's name — search over a grid of cards, each with its picture, name and handle — and each component opens onto its page: pictures, the install command, what it is, its own files and the places it reaches into, tags, packages, where it is on screen, and its code in tabs with line numbers, opening at the line an entry points to. Agents use it from their shell with the `ruri` command on every project session's PATH: `ruri search`, `ruri list`, `ruri show <slug>` (with code), `ruri add <slug> --dir <folder>` (copies it into place; the folder is remembered; `ruri add <project>/<slug>` takes one from another open project), `ruri register`, `ruri edit`, `ruri remove`, `ruri dir`. Claude sessions carry it as the `ruri:components` skill, whose description never changes so the prompt stays cached. Interface only: the sweep names interface and nothing else, `ruri register` refuses backend files, and entries from the old index with no interface in them are set aside in `components/<projectId>.not-ui.json`. Harnesses without ruri's tools register with `ruri register` instead of the `.ruri/components.jsonl` drop file
