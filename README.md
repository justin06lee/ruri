...
- **Feature tracker** — the same small model watches each turn and extracts new user-visible features/changes into a per-project checklist (drawer via the header button; it pops open when new items land), so nothing from a long prompt gets forgotten. Reviewing is one pass: click anywhere on an item — once for works (check), twice for needs-work (x), which smoothly folds out a note field (foldable again to stay focused) — and items never move while you review. Paste files straight into a note; they show as little numbered bumps on the textbox's top edge (click one to remove it). **Finish review** then clears the checked items, reopens the crossed ones pinned at the top with a repeat badge on their checkbox (new extractions land below them), and has the small model write one fix-it prompt from the crossed items and their notes — note attachments ride along as stored file paths for tool inspection — dropped straight into the composer for you to send. Clicking anywhere outside the drawer closes it. Persists in `~/.config/ruri/tracker/`.
