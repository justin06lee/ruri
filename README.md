...
- **A manga look** on warm paper, three themes on a clock, the dragon gauges for context and account limits, the peek band in the title bar (the hand-cut heads, or any pictures and GIFs you give it, each with its own hover), the marks of whoever made the chat's model doodled over every empty chat (Anthropic × Claude, OpenAI × ChatGPT, Google × Gemini and the rest), a music player in the sidebar, and an art tuner.
