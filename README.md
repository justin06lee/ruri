...
- **Projects sidebar** with optional group labels, add/remove, and status dots (filled pulse = working, thick ring = waiting on a permission, diamond = error/unread). Adding a project opens the native macOS folder picker (browser dev mode falls back to typing a path).
