...
- **Home — the workspace agent.** The default view (and the pinned Home row) is a session that manages the app itself: tell it "today let's work on X, Y, Z" and it finds the matching directories under your workspace root, opens them in the sidebar, and can kick each session off with a delegated first prompt. On Claude that happens through an in-process `open_project` MCP tool; on any other harness Home drops open requests into `.ruri/open.jsonl` at the workspace root and ruri applies them as the turn ends — so Home orchestrates on whatever model you point it at. It also carries a touch of its namesake's personality (RuriDragon's Aoki Ruri: deadpan, unbothered, gets it done anyway). The workspace root is configurable from Settings; deep work happens in each project's own session, Home just orchestrates. Its chat is deliberately ephemeral — every app launch starts it blank, and navigating away and back wipes it too (a running turn finishes first): it's a dispatcher, not a place to accumulate context. It also skips the header bar — nothing between the titlebar skyline and the conversation.
- **Manga look** — Space Grotesk type, pure black-and-white UI on warm paper (deliberately low blue channel, e-ink vibes): ink borders, screentone shading, hard offset shadows, grayscale syntax highlighting that leans on weight and slant instead of hue. Statuses are shapes, not colors — the Home row keeps a status dot (hollow = idle, filled pulse = working, thick pulsing ring = waiting on permission, diamond = error), session rows stay clean, and a small diamond pip appears on a row only when a background turn *finishes*. Scrollbars are hairline-thin (3px) everywhere, and while a turn thinks, a two-frame hand-drawn Ruri doodle flips poses in the transcript.
- **Projects sidebar as folders of sessions**: each project is a collapsible folder holding any number of parallel sessions, and each session is auto-named by the small model the moment its first prompt goes out (in parallel with the turn, not after it) — by the ROLE it serves ("Frontend UI", "Backend API"), not the literal prompt. Star projects to pin them into a Starred section; + on a folder adds a session. Collapsed folders stay collapsed across restarts. Removing a session deletes its transcript, and removing the last one closes the project — files on disk are never touched. Projects are opened by asking Home.
...
- **Attachments** — drop or paste anything into the composer (25MB each): images, videos, PDFs, text/source files, whatever. A `[image #N]`/`[video #N]`/`[file #N]` marker lands in the text (with a space after it, so typing continues cleanly) with thumbnails above it — non-media files show a doc tile with their extension; click any thumbnail for the full-size view (PDFs render inline, text files show their contents). Drag on a full-size image to mark regions and attach a note to each — every region is cropped and sent as its own image so the model sees exactly the part you meant. Images go to the model directly; videos and other files are saved to disk and referenced by path for tool-based inspection. Uploads persist under `~/.config/ruri/uploads/`. Unsent drafts — text and attachments both — are kept per session, so switching sessions mid-composition and coming back restores exactly what you were writing.
...
- **Interrupt** button to stop a running turn, and a transcript that follows output only while you're at the bottom (a jump-to-latest pill floats just above the composer otherwise).
- **Usage gauges** — three little vials above the account bar show what's *left*: the account's 5-hour and weekly limit windows (read via the signed-in Claude Code OAuth token from the same endpoint the CLI's own `/usage` uses — keychain on macOS, `~/.claude/.credentials.json` elsewhere; polled every 5 minutes and after each turn) and the active session's context window (remaining tokens out of 1M for `[1m]` models, 200k otherwise, measured from the latest API call's token usage). No token or a non-Claude session just leaves a gauge empty.
- **Music player** in the sidebar (ported from `home`): point Settings → Music at any folder (default `~/Music/ruri`, or `RURI_MUSIC_DIR`) — each subfolder is a playlist, loose files are "Unsorted". Two-deck Web Audio engine with 6s equal-power crossfade, shuffle, seek, volume; tracks stream from ruri's own server with Range support. While a track plays, a five-bar waveform fed by a real analyser tap on the master output rides the gap between the track title and the chevron (the note icon stays put), and little notes wobble upward from the player bar.
...
- **A different Ruri every time** — the hero face above the composer is drawn from a pool of 13 cropped Ruri panels: Home rolls a fresh one each app launch, and every project keeps the face it was born with. Up in the title bar sits the **peek skyline**: five hand-cut head PNGs (background removed and missing parts redrawn by hand) resting low in the bar, just horns and hair showing — hover one and she lifts slightly, just enough to see her face. The whole bar stays a window-drag region — the desktop app's main process polls the cursor to drive the hover, since drag regions never deliver mouse events to the page.
- **Feature tracker** — the same small model watches each turn and extracts new user-visible features/changes into a per-project checklist (drawer via the header button; it pops open when new items land), so nothing from a long prompt gets forgotten. Reviewing is one pass: click a box once for works (check), twice for needs-work (x) — which smoothly folds out a note field (foldable again to stay focused) — and items never move while you review. **Finish review** then clears the checked items, reopens the crossed ones pinned at the top with a repeat mark (new extractions land below them), and has the small model write one fix-it prompt from the crossed items and their notes, dropped straight into the composer for you to send. Any item can still be sent to the composer alone. Persists in `~/.config/ruri/tracker/`.
- **Turn memory & instant compaction** — after every finished turn, a small model writes a terse recall note for that prompt/response pair. Which model: star one twice in the Settings catalog to crown it the **small-tasks model** (an inverted "small tasks" tag marks it; any harness works — yagami routes qualified ids), used everywhere the small model runs (recall notes, session role titles, prompt splitting, the tracker); with nothing double-starred it's `RURI_SMALL_MODEL` or Haiku, and `RURI_NO_MEMORY=1` disables the layer. Older turns fold to their notes automatically; clicking one pulls the full turn back. Because notes are precomputed per turn, "compaction" costs nothing at read time. Transcripts, notes, and the resumable session id persist in `~/.config/ruri/sessions/`, so history and context survive app restarts.
...
- `server/archive.ts` — `SessionArchive`: transcript events, per-turn recall notes, and the resumable session id per session, debounce-written to disk. The source of truth behind the connect snapshot and instant compaction.
- `server/smallmodel.ts` — the small-tasks layer over yagami's completions client: turn summaries, session role titles, prompt splitting, tracker extraction, plus `TurnTracker`, which assembles prompt→result turns out of the event stream.
...
- `server/usage.ts` — the usage gauges' account side: the Claude Code OAuth token (keychain / credentials file) and the 5-hour / weekly limit-window fetch, all best-effort.
...
- `web/src/components/Sidebar.tsx` — Home row, project folders and their sessions, the peek skyline, the account bar.
...
| `sessions/<sessionId>.json` | Transcript events, per-turn recall notes, resumable session id |
