...
- **Edit & rewind** — hover any past prompt in an idle Claude session and a pencil appears on its shoulder; click it and a card asks the real question: rewinding returns the conversation *and the project's files* to the moment before that prompt ran. Confirm and the CLI restores its file checkpoints (sessions run with checkpointing on), the session resumes truncated — forked at the kept turn's last message, so the original chain survives on disk — the discarded turns leave the transcript, and the old prompt lands back in the composer for editing and re-sending. Needs yagami ≥ 0.6.1 (which exposes the CLI's `/rewind`); attachments don't ride back, and rewinds don't cross a compaction.
- **Rapid fire** — the bolt row under Home flips the main pane into assembly-line prompting for parallel work. It always shows exactly one thing: the next session that's ready for a prompt (its last exchange as context, plus any pending permission card), with a composer. Send and the card yields to the next ready session; a session that finishes its turn rejoins the line at the back, so by the time you've prompted the last one the first is usually waiting again. Skip passes a session to the back; when everyone's working, the page just waits with the doodle until someone finishes.
...
- **Turn memory** — after every finished turn, a small model writes a terse recall note for that prompt/response pair. Which model: star one twice in the Settings catalog to crown it the **small-tasks model** (an inverted "small tasks" tag marks it; any harness works — yagami routes qualified ids), used everywhere the small model runs (recall notes, session role titles, prompt splitting, the tracker); with nothing double-starred it's `RURI_SMALL_MODEL` or Haiku, and `RURI_NO_MEMORY=1` disables the layer. The notes stay out of the way — turns always show in full; a hover chevron on a turn's top-left folds it down to its note, and clicking the folded card pulls it back. Transcripts, notes, and the resumable session id persist in `~/.config/ruri/sessions/`, so history and context survive app restarts.
- **ruri's own `/compact`** — type `/compact` and ruri compacts the session itself instead of letting the harness do it, instantly and at zero model cost: the live session is retired (resume id cleared), every prior exchange's full record — prompt, response, tool activity — is written to `~/.config/ruri/turns/<session>/NNN.md`, and a brief is built from the precomputed recall notes, each entry ending with its exchange's file path so the fresh model can Read the full record whenever a summary isn't detail enough. The brief rides invisibly on your next prompt into the brand-new session. You never see any of that: the transcript keeps your full history and just shows a jagged torn line — "compacted" — at the break (click the label if you're curious what the model was handed). Works on any harness, since it's all app-side.
...
- `server/compaction.ts` — ruri's own `/compact`: writes each exchange's full record to `turns/<session>/NNN.md` and builds the model-facing brief out of the precomputed recall notes, one file hook per entry. No model call — it's instant.
...
- `web/src/components/RapidFire.tsx` — the rapid-fire pane: a client-side line of prompt-ready sessions, reusing the transcript pieces and composer.
