...
- **A project's memory and architecture** — every chat is pointed at `.ruri/catchup.md` (git's own account of where things stand, then decisions and why, what worked, what failed and why, the traps, what's open — each line with the day it was learned, the exchange it came from and who wrote it) and `.ruri/architecture.md` (where to change what, the stack as layers, the flows through it, where things are). Agents add what they learn with `ruri note`, search every past exchange with `ruri recall`, and read git and their own changes with `ruri state`; the Architecture page draws both files, and there you correct, pin or strike any line.
