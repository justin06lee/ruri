...
import { BRIEF_LISTED, buildCompaction, DigestFolder, dueForDigest, type FoldDigest } from "../server/compaction.js";
...
check("one past it folds the oldest down to 30 left", dueForDigest(ids(41), undefined).join() === ids(11).join());
check(
...
check("95 exchanges fold in two goes, 1–40 then 41–65", JSON.stringify(calls.map((c) => [c[0], c.at(-1)])) === "[[1,40],[41,65]]", calls.map((c) => [c[0], c.at(-1)]));
...
const built = buildCompaction(channel, archive.allEvents(channel), archive.summaries(channel), archive.digest(channel));
check("the brief lists only the 30 after the digest", built?.entries.length === 30, built?.entries.length);
check("numbered as in the whole conversation", built?.entries[0]?.n === 66 && built?.entries.at(-1)?.n === 95);
check("the brief opens on the digest, naming the records", Boolean(built?.brief.includes("<condensed>") && built.brief.includes("Exchanges 1–65") && built.brief.includes("065.md")));
check("the digest's exchanges are not listed again", !built?.brief.includes("\n1. user:") && Boolean(built?.brief.includes("\n66. user: ask 66")));
check("the mark carries the digest for the window", built?.digest?.through === 65);
check("every exchange keeps its record on disk", fs.existsSync(path.join(configDir, "turns", channel, "095.md")));

calls.length = 0;
for (let n = 96; n <= 105; n++) exchange(n);
await folder.run(channel);
check("40 listed after the digest is still within the cap", calls.length === 0 && archive.turnIds(channel).length === 105);
exchange(106);
await folder.run(channel);
check("the 41st folds the oldest 11 back down to 30", JSON.stringify(calls) === JSON.stringify([ids(11, 66).map((id) => Number(id.slice(1)))]), calls);
...
release?.();
await running;
check("a fold finishing after a rewind took its exchanges is dropped", archive.digest(channel) === undefined, archive.digest(channel));
...
check("no digest: every exchange listed from 1", plain?.entries[0]?.n === 1 && !plain.brief.includes("<condensed>"));
