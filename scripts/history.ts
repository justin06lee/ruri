...
/* a compaction folds the past away */
{
  ...
  check(
    "the live part opens on the mark",
    ids(archive.events("s1")) === "c1,u3,a3,r3",
    ids(archive.events("s1")),
  );
  check("the history holds what came before", ids(archive.history("s1")) === "u1,a1,r1,u2,a2,r2");
  check(
    "the whole conversation reads in order",
    ids(archive.allEvents("s1")) === "u1,a1,r1,u2,a2,r2,c1,u3,a3,r3",
  );
...
/* an old single-file archive is split on load, and a crash heals */
{
  ...
  const archive = new SessionArchive();
  check(
    "a legacy archive is split at its newest mark",
    ids(archive.events("s2")) === "c2,u3,a3,r3",
    ids(archive.events("s2")),
  );
  check(
    "without writing anything twice",
    ids(archive.history("s2")) === "u1,a1,r1,c1,u2,a2,r2",
    ids(archive.history("s2")),
  );
...
/* a rewind into the history */
{
  ...
  check(
    "what is kept is split again at its own newest mark",
    ids(archive.events("s2")) === "c1" && ids(archive.history("s2")) === "u1,a1,r1",
  );
...
/* a fork gets its own split */
{
  ...
  check(
    "a seeded fork has its own history",
    ids(archive.history("s3")) === "u1,a1,r1" && ids(archive.events("s3")) === "c1,u2,a2,r2",
  );
}
...
/* the cap */
{
  ...
  check(
    "it keeps the newest exchanges",
    history[history.length - 1]?.id === "u39" || history.some((e) => e.id === "u39"),
  );
  check(
    "the oldest are gone, and their notes with them",
    !history.some((e) => e.id === "u0") && archive.summaries("s4")["u0"] === undefined,
  );
