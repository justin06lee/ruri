...
check(
  ...
  offTask(
    "This isn't a message to compress—it's a feature request and formatting complaint. Provide the actual user messages that need compaction.",
    META_PROMPT,
  ),
);
check(
  ...
  offTask(
    "I can't see images, and these questions are outside Google Drive—I only have file management tools here.",
    "[image #1] fix this [image #2] and this",
  ),
);
check(
  "a question the message never asked is off task",
  offTask(
    "Can you share the file ID or link to the code for this feature?",
    "find the naming components feature and tell me where it lives",
  ),
);
check(
  "a note longer than its source is off task",
  offTask(
    "fix the header flicker on scroll and also shrink the logo a bit while you are at it please",
    "fix header flicker",
  ),
);
check(
  "a real compression passes",
  !offTask(
    "header flickers on scroll; shrink logo",
    "hey so when I scroll down the page the header kind of flickers? could we make the logo smaller too",
  ),
);
check(
  "the job's words are fine when the message used them",
  !offTask(
    "fix compaction model so notes don't do this; small model changeable?",
    "can you fix the compaction model so it doesn't do this? it's just using a small model that I can change right?",
  ),
);
...
check(
  "a message about compression still gets compressed",
  metaNote.length > 0 && !offTask(metaNote, META_PROMPT),
  metaNote,
);
check("and the note keeps its items", /music/i.test(metaNote) && /nam/i.test(metaNote), metaNote);

const ADDRESSED =
  "[image #1] I can't see what you changed. Share the file or tell me where it's located (GitHub repo, Google Drive folder, etc.) and I'll compress your request for the developer.";
const addressedNote = await summarizePrompt(ADDRESSED);
console.log("    addressed prompt →", JSON.stringify(addressedNote));
check(
  "a message that reads as addressed to the helper is compressed, not answered",
  addressedNote.length > 0 && !offTask(addressedNote, ADDRESSED),
  addressedNote,
);
