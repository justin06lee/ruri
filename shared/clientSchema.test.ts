...
const samples: { [K in ClientMessage["type"]]: Extract<ClientMessage, { type: K }> } = {
  ...
  queue_update: { type: "queue_update", projectId: "p", itemId: "i", text: "t", attachments: [upload], split: true },
  ...
  draft: {
    ...
    projectId: "p",
    text: "half a thought",
    attachments: [{ id: "d", kind: "file", mediaType: "text/plain", name: "a.txt", n: 1, regions: [{ x: 1, y: 2, w: 3, h: 4, n: 2 }] }],
  },
  ...
  question_response: {
    ...
    requestId: "r",
    answers: { answers: { q: "yes" }, annotations: { q: { notes: "n" } }, response: "ok", values: { q: ["a", "b"] } },
  },
  ...
  component_named: { type: "component_named", requestId: "r", name: "Header", files: ["a.tsx"], note: "n", skip: false },
  component_update: {
  ...
  tracker_update: { type: "tracker_update", projectId: "p", itemId: "i", status: "liked", note: "n", text: "t" },
...
describe("malformed messages are refused, with a path", () => {
  const cases: Array<[string, unknown, string]> = [
    ...
    ["a bad attachment inside a list", { type: "send", projectId: "p", text: "t", attachments: [{ ...upload, kind: "exe" }] }, "attachments.0.kind"],
    ["an attachment number below zero", { type: "tracker_attach", projectId: "p", itemId: "i", upload: { ...upload, n: -1 } }, "upload.n"],
    ["a permission mode that does not exist", { type: "set_permission_mode", projectId: "p", mode: "yolo" }, "mode"],
    ...
    ["a string for a terminal size", { type: "terminal_open", projectId: "p", termId: "t", cols: "80", rows: 24 }, "cols"],
    ["a skill scope that does not exist", { type: "skill_toggle", scope: "system", name: "x", on: true }, "scope"],
    ["an answer that is not a string", { type: "question_response", requestId: "r", answers: { answers: { q: 1 } } }, "answers.answers.q"],
