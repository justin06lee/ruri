...
const askAnswers = z.object({
  ...
  annotations: z
    .record(z.string(), z.object({ preview: z.string().max(100_000).optional(), notes: z.string().max(100_000).optional() }))
    .optional(),
...
export const clientMessageSchema: z.ZodType<ClientMessage> = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("send"), ...projectId, text, attachments: z.array(attachmentUpload).optional() }),
  z.object({ type: z.literal("send_split"), ...projectId, text, attachments: z.array(attachmentUpload).optional() }),
  ...
  z.object({ type: z.literal("view"), channels: z.array(id).max(1_000), live: z.boolean(), board: z.boolean().optional() }),
  z.object({ type: z.literal("history_get"), ...projectId }),
  z.object({ type: z.literal("recent_import"), ...projectId, id }),
  z.object({ type: z.literal("draft"), ...projectId, text, attachments: z.array(draftAttachmentUpload).optional() }),
  ...
  z.object({ type: z.literal("terminal_open"), ...projectId, termId: id, cols: z.number(), rows: z.number() }),
  z.object({ type: z.literal("terminal_input"), ...projectId, termId: id, data: z.string().max(1_000_000) }),
  z.object({ type: z.literal("terminal_resize"), ...projectId, termId: id, cols: z.number(), rows: z.number() }),
  z.object({ type: z.literal("terminal_close"), ...projectId, termId: id }),
  z.object({ type: z.literal("permission_response"), requestId: id, allow: z.boolean(), always: z.boolean().optional() }),
  ...
  z.object({ type: z.literal("idea_update"), ...projectId, ideaId: id, text: text.optional(), done: z.boolean().optional() }),
  ...
  z.object({ type: z.literal("skill_toggle"), ...optionalProjectId, scope: skillScope, name: label, on: z.boolean() }),
