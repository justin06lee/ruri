...
export const clientMessageSchema: z.ZodType<ClientMessage> = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    files: z.array(filePath).max(1_000).optional(),
    uses: z.array(filePath).max(1_000).optional(),
    tags: z.array(label).max(100).optional(),
    deps: z.array(label).max(200).optional(),
    note: text.optional(),
  ...
  z.object({
    type: z.literal("layer_line"),
    ...projectId,
    slug: id,
    section: z.enum(["summary", "map", "files", "rules", "edges"]),
    index: z.number().int().min(0).max(1000),
    text: label.min(1).optional(),
  }),
