...
export function installFixture(): void {
  ...
  useRuri.setState({
    ...
    transcripts: {
      ...
      p1: [
        ...
        { kind: "tool", id: "e5", name: "Bash", summary: "bun run typecheck && bun test", ts: now - 60_000 },
        {
          ...
          id: "e5a",
          name: "Agent",
          summary: "Audit every reconnect path in the client",
          agent: {
            key: "toolu_fixture_1",
            type: "Explore",
        ...
        },
        {
          ...
          id: "e5b",
          name: "Agent",
          summary: "Run the whole suite against the fix",
          agent: {
            key: "toolu_fixture_2",
        ...
        },
        {
          kind: "plan",
          id: "provider-plan",
          explanation: "The harness is keeping this plan current while it works.",
          entries: [
            { content: "Verify the reconnect race", status: "completed" },
            { content: "Exercise the native question flow", status: "in_progress" },
            { content: "Run the production build", status: "pending" },
          ],
          ts: now - 10_000,
        },
      ],
        ...
        input: {
          questions: [
            ...
            },
            {
              ...
              header: "Token",
              hint: "The harness receives this answer, but ruri never writes it into the transcript.",
              inputType: "string",
              required: true,
              secret: true,
              allowOther: true,
    ...
    // what the two agents in p1 did, as their opened cards show it
    agentLogs: {
      "p1\u0000toolu_fixture_1": [
        {
          kind: "user",
          id: "toolu_fixture_1:brief",
        ...
        { kind: "tool", id: "a2", name: "Read", summary: "ruri/web/src/store.ts", ts: now - 57_000 },
        {
          kind: "assistant",
          id: "a3",
          text: "The client opens its socket in one place, `connect()`, and reaches it from three. Checking the server side for the snapshot path.",
          ts: now - 56_000,
        },
        { kind: "tool", id: "a4", name: "Read", summary: "ruri/server/server.ts", ts: now - 55_000 },
        {
          kind: "assistant",
          id: "a5",
          text: "Three paths reconnect:\n\n1. `connect()` on load — `web/src/store.ts:604`\n2. `ws.onclose` after a drop — `web/src/store.ts:610`, the only one that backs off\n3. the snapshot handler after a server restart — `web/src/store.ts:620`\n\nNone of them resets `attempt` on a clean open.",
          ts: now - 52_000,
        },
      ],
      "p1\u0000toolu_fixture_2": [
        {
          kind: "user",
          id: "toolu_fixture_2:brief",
          text: "Run every test script in package.json against the reconnect fix and report anything that fails.",
          ts: now - 40_000,
        },
